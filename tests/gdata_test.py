...
class StartIndexTest(unittest.TestCase):
    ...
    def testToAndFromString(self):
        ...
        self.assertTrue(self.start_index.text.encode() == new_start_index.text)
...
class ItemsPerPageTest(unittest.TestCase):
    ...
    def testToAndFromString(self):
        ...
        new_items_per_page = gdata.ItemsPerPageFromString(self.items_per_page.ToString())
        self.assertTrue(self.items_per_page.text.encode() == new_items_per_page.text)
...
class GDataEntryTest(unittest.TestCase):
    def testIdShouldBeCleaned(self):
        ...
        self.assertTrue(element_tree.findall('{http://www.w3.org/2005/Atom}id')[0].text != entry.id.text)
        self.assertTrue(entry.id.text == b'http://www.google.com/test/id/url')
    ...
    def testGeneratorShouldBeCleaned(self):
        ...
        self.assertTrue(element_tree.findall('{http://www.w3.org/2005/Atom}generator')[0].text != feed.generator.text)
        self.assertTrue(feed.generator.text == b'GoogleBase')
...
class LinkFinderTest(unittest.TestCase):
    def setUp(self):
        ...
        #     # self.assertEqual(isinstance(self.entry.GetLicenseLink(), atom.Link), True)
        #     self.assertEqual(self.entry.GetLicenseLink().href, 'http://creativecommons.org/licenses/by-nc/2.5/rdf')
        #     self.assertEqual(self.entry.GetLicenseLink().rel, 'license')

        # def testLinkFinderGetsAlternateLink(self):
        #     self.assertEqual(isinstance(self.entry.GetAlternateLink(), atom.Link), True)
        #     self.assertEqual(self.entry.GetAlternateLink().href, 'http://www.provider-host.com/123456789')
...
class BatchEntryTest(unittest.TestCase):
    def testCorrectConversionFromAndToString(self):
        ...
        self.assertEqual(batch_entry.batch_id.text, b'itemB')
        self.assertEqual(batch_entry.id.text, b'http://www.google.com/base/feeds/items/2173859253842813008')
        self.assertEqual(batch_entry.batch_operation.type, b'insert')
        self.assertEqual(batch_entry.batch_status.code, b'201')
        self.assertEqual(batch_entry.batch_status.reason, b'Created')
...
class BatchFeedTest(unittest.TestCase):
    ...
    def testConvertRequestFeed(self):
        ...
        self.assertEqual(batch_feed.title.text, b'My Batch Feed')
    ...
    def testConvertResultFeed(self):
        ...
        for entry in batch_feed.entry:
            self.assertTrue(isinstance(entry, gdata.BatchEntry))
            if entry.id.text == ('http://www.google.com/base/feeds/items/2173859253842813008'):
        ...
        self.assertEqual(batch_feed.title.text, b'My Batch')
...
class ExtendedPropertyTest(unittest.TestCase):
    ...
    def testGettersReturnCorrectTypes(self):
        ...
        ep.SetXmlBlob('<some_xml attr="test"/>')
        self.assertTrue(isinstance(ep.GetXmlBlobExtensionElement(), atom.ExtensionElement))
        self.assertTrue(isinstance(ep.GetXmlBlobString(), bytes))
...
class FeedLinkTest(unittest.TestCase):
    def testCorrectFromStringType(self):
        ...
        self.assertEqual(link.count_hint, b'5')
