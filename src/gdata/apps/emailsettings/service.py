...
class EmailSettingsService(gdata.apps.service.PropertyService):
    ...
    def CreateFilter(self, username, from_=None, to=None, subject=None,
        ...
        properties = {'from': from_, 'to': to, 'subject': subject, 'hasTheWord': has_the_word, 'doesNotHaveTheWord': does_not_have_the_word,
                      'hasAttachment': gdata.apps.service._bool2str(has_attachment), 'label': label,
                      'shouldMarkAsRead': gdata.apps.service._bool2str(should_mark_as_read), 'shouldArchive': gdata.apps.service._bool2str(should_archive)}
        return self._PostProperties(uri, properties)
    ...
    def CreateSendAsAlias(self, username, name, address, reply_to=None,
        ...
        properties = {'name': name, 'address': address, 'replyTo': reply_to, 'makeDefault': gdata.apps.service._bool2str(make_default)}
        return self._PostProperties(uri, properties)
