        ...
        if auth_type == CLIENT_LOGIN:
            ...
            if private_key is not None:
                if client.auth_token is None:
                    if session_token:
                        client.auth_token = gdata.gauth.SecureAuthSubToken(session_token, private_key, scopes)
                        self.prefs['client_auth_token'] = gdata.gauth.token_to_blob(
                            client.auth_token)
                        return
                    elif auth_sub_token:
                        client.auth_token = gdata.gauth.SecureAuthSubToken(auth_sub_token, private_key, scopes)
                        client.upgrade_token()
                        self.prefs['client_auth_token'] = gdata.gauth.token_to_blob(
                            client.auth_token)
                        return

                auth_url = gdata.gauth.generate_auth_sub_url('http://gauthmachine.appspot.com/authsub', scopes, True)
                ...
                if client.auth_token is None:
                    if session_token:
                        ...
                        client.upgrade_token()
                        self.prefs['client_auth_token'] = gdata.gauth.token_to_blob(client.auth_token)
                        return
            ...
            'Visit the following URL in your browser to authorize this app:'
            print()
            str(request_token.generate_authorization_url())
            print()
            'After agreeing to authorize the app, copy URL from the browser\'s'
            print()
            ' address bar.'
